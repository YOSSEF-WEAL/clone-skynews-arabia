...
export async function GET(request, { params }) {
  ...
  try {
    ...
    // Determine tag ID (support slug or ID)
    let tagId = tag;
    if (!/^\d+$/.test(String(tag))) {
      const resolveUrl = `${wpBase}/wp-json/wp/v2/tags?slug=${encodeURIComponent(
        tag
      )}&_fields=id&per_page=1`;
      const resolveRes = await fetch(resolveUrl, { headers: wpHeaders });
      if (!resolveRes.ok) {
        return new NextResponse(
          JSON.stringify({ error: "Failed to resolve tag slug" }),
          { status: 502, headers: { "Content-Type": "application/json" } }
        );
      }
      const tags = await resolveRes.json();
      tagId = tags?.[0]?.id;
      if (!tagId) {
        return new NextResponse(JSON.stringify({ error: "Tag not found" }), {
          status: 404,
    ...
    const response = await fetch(`${postsUrl}?${paramsQS}`, {
      headers: wpHeaders,
    });
