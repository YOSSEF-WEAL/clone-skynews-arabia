// Using internal API route for search functionality
import { headers as nextHeaders } from "next/headers";

const apiSearchPosts = async (searchQuery, page = 1, perPage = 20) => {
  try {
    // Guard: if search query is missing/invalid, do not call the API
    if (
      ...
      console.warn(
        "[apiSearchPosts] skipped due to invalid searchQuery:",
        searchQuery
      );
      return {
        posts: [],
    ...
    const encodedQuery = encodeURIComponent(String(searchQuery).trim());
    const requestUrl = `${baseUrl}/api/posts/search?q=${encodedQuery}&per_page=${perPage}&page=${page}`;
    ...
    if (!res.ok) {
      // If internal route returns 404 (e.g., no results), do not break the page
      const errText = await res.text().catch(() => "");
      console.error(
        "[apiSearchPosts] HTTP error",
        res.status,
        requestUrl,
        errText
      );
    ...
    console.error("Error loading search results from API:", error);
    return {
      posts: [],
      pagination: { currentPage: 1, totalPages: 0, totalItems: 0 },
    };
